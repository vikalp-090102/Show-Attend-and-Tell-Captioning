...
def train_collate_fn(batch):
    """
    Expects: list of (img_tensor, cap_ids : torch.LongTensor)
    Returns:
        images: (B, C, H, W)
        padded_caps: (B, max_len) padded caption indices
        lengths: list of actual caption lengths
    """
    # sort batch by descending caption length
    ...
    for i, cap in enumerate(caps):
        padded[i, :lengths[i]] = cap[:lengths[i]]
    return images, padded, lengths

def val_collate_fn(batch):
    """
    Expects: list of (img_tensor, cap_ids : torch.LongTensor, all_ref_caps : List[List[int]])
    Returns:
        images, padded_caps, lengths, all_caps (as a nested list)
    """
    batch.sort(key=lambda x: len(x[1]), reverse=True)
...
def main(args):
    ...
    # --- Vocabulary handling ---
    # word_dict.json should be stored at args.data; if not, auto-generate it.
    ...
    if os.path.exists(word_dict_path):
        ...
        # Create a temporary training dataset instance (without word2idx supplied)
        train_temp_ds = ImageCaptionDataset(transform=data_transforms,
                                            data_path=args.data,
                                            split_type='train')
        counter = Counter()
        for cap in train_temp_ds.captions:
            # Process impressions all in lower case
            counter.update(cap.lower().split())
        tokens = list(counter.keys())
        # Reserve special tokens: you can always adjust as needed.
    ...
    # Note: ImageCaptionDataset is expected to use the CSV files:
    #   indiana_projections.csv and indiana_reports.csv
    # and it will load images from:
    #   <args.data>/images/images_normalized/<image_filename>
...
def train(epoch, encoder, decoder, optimizer, criterion, data_loader, word_dict, alpha_c, log_interval, writer):
    encoder.eval()   # In our setup the encoder is pre-trained; we keep it in eval mode.
    decoder.train()
...
# -----------------------------------------------------------------------------
# Validation loop (also computes BLEU scores)
# -----------------------------------------------------------------------------
def validate(epoch, encoder, decoder, criterion, data_loader, word_dict, alpha_c, log_interval, writer):
    ...
    with torch.no_grad():
        for bi, (imgs, caps, lengths, all_caps) in enumerate(data_loader):
            ...
            # Gather reference captions (each image can have multiple)
            ...
            # Build hypothesis captions from decoder outputs
...
if __name__ == "__main__":
    ...
    # For the Indiana dataset, --data should point to the folder containing:
    # indiana_projections.csv, indiana_reports.csv, and the images/ folder.
    parser.add_argument('--data', type=str, default='/kaggle/input/chest-xrays-indiana-university',
                        help='path to data')
    parser.add_argument('--network', choices=['vgg19', 'resnet152', 'densenet161'], default='vgg19',
                        help='Network to use in the encoder')
